...
var (
	 ocmControllerRepoName  = "ocm-controller-repo"
	 ocmControllerNamespace = "ocm-system"
)
...
func TestHappyPath(t *testing.T) {
	...
	management := features.New("Configure Management Repository").
		...
		Setup(setup.AddGitRepository(ocmControllerRepoName)).
		//TODO: change this to add single gitsource and
		// multiple kustomizations
		Setup(setup.AddFluxSyncForRepo(ocmControllerRepoName, "projects/", namespace)).
		Assess("project flux resources have been created", checkFluxResourcesReady(ocmControllerRepoName)).
		Setup(setup.CreateNamespace(ocmControllerNamespace))
...
func checkFluxResourcesReady(name string) features.Func {
	fmt.Println("Check if Flux Resources are ready")
